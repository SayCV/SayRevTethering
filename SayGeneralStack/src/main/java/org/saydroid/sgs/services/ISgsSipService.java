...
/**@page SgsSipService_page SIP/IMS Service
 ...
 * @code
 * final ISgsSipService mSipService = SgsEngine.getInstance().getSipService();
 * @endcode
 * 
 ...
 * final String validUri = SgsUriUtils.makeValidSipUri(remoteUri); // sip:+33600000000"@saydroid.org
 * SgsAVSession avSession = SgsAVSession.createOutgoingSession(mSipService.getSipStack(), SgsMediaType.Audio);
 ...
 * final String remoteUri = "+33600000000";
 * final String validUri = SgsUriUtils.makeValidSipUri(remoteUri); // sip:+33600000000"@saydroid.org
 * SgsAVSession avSession = SgsAVSession.createOutgoingSession(mSipService.getSipStack(), SgsMediaType.AudioVideo);
 * if(avSession.makeCall(validUri)){
 * 	Log.d(TAG,"all is ok");
 * }
 * else{
 ...
 * @code
 * final String SMSC = "sip:+3310000000@saydroid.org"; // SMS Center
 * final String remotePartyUri = "sip:+336000000@saydroid.org"; // remote party
 * final String textToSend = "hello world!";
 * final SgsMessagingSession imSession = SgsMessagingSession.createOutgoingSession(mSipService.getSipStack(), 
 * remotePartyUri);
 ...
 * }
 * // release session
 * SgsMessagingSession.releaseSession(imSession);
 * @endcode
 * 
 * <h2>Pager Mode IM</h2>
 * @code
 ...
 * 			InviteState state;
 * 			final String action = intent.getAction();
 * 			if(SgsInviteEventArgs.ACTION_INVITE_EVENT.equals(action)){
 * 				SgsInviteEventArgs args = intent.getParcelableExtra(SgsEventArgs.EXTRA_EMBEDDED);
 * 				if(args == null){
 * 					Log.e(TAG, "Invalid event args");
 * 					return;
 * 				}
 ...
 * 				SgsAVSession avSession = SgsAVSession.getSession(args.getSessionId());
 * 				if(avSession == null){
 ...
 * @code 
 * final String myRealm = "sip:saydroid.org"; 
 ...
 * @code
 * final boolean bSaveNow = true; 
 * final String myIMPU = "sip:33446677887@saydroid.org"; 
 ...
 * The Proxy-CSCF Host is the IP address (192.168.0.1) or FQDN (saydroid.org) of the SIP registrar. <br />
 ...
 * @endcode
 */
public interface ISgsSipService extends ISgsBaseService {
