...
import org.saydroid.sgs.media.SgsProxyAudioConsumer;
import org.saydroid.sgs.media.SgsProxyAudioProducer;
import org.saydroid.sgs.media.SgsProxyPlugin;
import org.saydroid.sgs.media.SgsProxyPluginMgr;
import org.saydroid.sgs.media.SgsProxyVideoConsumer;
import org.saydroid.sgs.media.SgsProxyVideoProducer;
import org.saydroid.sgs.model.SgsHistoryAVCallEvent;
 ...
 */
public class SgsAVSession extends SgsInviteSession{
	private static final String TAG = SgsAVSession.class.getCanonicalName();
	
	private CallSession mSession;
	private boolean mConsumersAndProducersInitialzed;
	private SgsProxyVideoConsumer mVideoConsumer;
	private SgsProxyAudioConsumer mAudioConsumer;
	private SgsProxyVideoProducer mVideoProducer;
	private SgsProxyAudioProducer mAudioProducer;
	private Context mContext;
	
	private final SgsHistoryAVCallEvent mHistoryEvent;
	private final ISgsConfigurationService mConfigurationService;
	
	private boolean mSendingVideo;
	
    private final static SgsObservableHashMap<Long, SgsAVSession> sSessions = new SgsObservableHashMap<Long, SgsAVSession>(true);
    
    public static SgsAVSession takeIncomingSession(SgsSipStack sipStack, CallSession session, twrap_media_type_t mediaType, SipMessage sipMessage){
        SgsMediaType media;

        synchronized (sSessions){
            switch (mediaType){
                case twrap_media_audio:
                    media = SgsMediaType.Audio;
                    break;
                case twrap_media_video:
                    media = SgsMediaType.Video;
                    break;
                case twrap_media_audiovideo:
                    media = SgsMediaType.AudioVideo;
                    break;
                default:
                    return null;
            }
            SgsAVSession avSession = new SgsAVSession(sipStack, session, media, InviteState.INCOMING);
            if (sipMessage != null){
    ...
    public static boolean handleMediaUpdate(long id, twrap_media_type_t newMediaType){
    	SgsAVSession avSession = SgsAVSession.getSession(id);
        if (avSession != null){
        	avSession.mConsumersAndProducersInitialzed = false;
        	avSession.initializeConsumersAndProducers();
            switch (newMediaType){
                case twrap_media_audio: avSession.mMediaType = SgsMediaType.Audio; return true;
                case twrap_media_video: avSession.mMediaType = SgsMediaType.Video; return true;
                case twrap_media_audiovideo: avSession.mMediaType = SgsMediaType.AudioVideo; return true;
     ...
     */
    public static SgsAVSession createOutgoingSession(SgsSipStack sipStack, SgsMediaType mediaType){
        synchronized (sSessions){
            final SgsAVSession avSession = new SgsAVSession(sipStack, null, mediaType, InviteState.INPROGRESS);
            sSessions.put(avSession.getId(), avSession);
            return avSession;
        }
    }
	
	public static void releaseSession(SgsAVSession session){
		synchronized (sSessions){
	...
	public static SgsObservableHashMap<Long, SgsAVSession> getSessions(){
		return sSessions;
	}
	 ...
	 */
	public static SgsAVSession getSession(long id) {
		synchronized (sSessions) {
	...
	public static SgsAVSession getSession(SgsPredicate<SgsAVSession> predicate) {
		synchronized (sSessions) {
	...
	public static int getSize(SgsPredicate<SgsAVSession> predicate) {
		synchronized (sSessions) {
    ...
    public static boolean hasActiveSession(){
    	synchronized (sSessions){
    		final Collection<SgsAVSession> mysessions = sSessions.values();
	    	for(SgsAVSession session : mysessions){
	    		if(session.isActive()){
	    			return true;
	    		}
	    	}
     ...
     */
    public static SgsAVSession getFirstActiveCallAndNot(long id){
		SgsAVSession session;
		for(Map.Entry<Long, SgsAVSession> entry : sSessions.entrySet()) {
     ...
     */
    public static boolean makeAudioCall(String remoteUri, SgsSipStack sipStack){
    	SgsAVSession avSession = SgsAVSession.createOutgoingSession(sipStack, SgsMediaType.Audio);
    	return avSession.makeCall(SgsUriUtils.makeValidSipUri(remoteUri));
    }
     ...
     */
    public static boolean makeAudioVideoCall(String remoteUri, SgsSipStack sipStack){
    	SgsAVSession avSession = SgsAVSession.createOutgoingSession(sipStack, SgsMediaType.AudioVideo);
    	return avSession.makeCall(SgsUriUtils.makeValidSipUri(remoteUri));
    }
    ...
    public boolean makeCall(String remoteUri){
        ...
        // action (call)
        int level = mConfigurationService.getInt(SgsConfigurationEntry.QOS_PRECOND_BANDWIDTH_LEVEL,
        		SgsConfigurationEntry.DEFAULT_QOS_PRECOND_BANDWIDTH_LEVEL);
    ...
    protected SgsAVSession(SgsSipStack sipStack, CallSession session, SgsMediaType mediaType, InviteState callState){
		super(sipStack);
        ...
        // Session timers
        if(mConfigurationService.getBoolean(SgsConfigurationEntry.QOS_USE_SESSION_TIMERS, SgsConfigurationEntry.DEFAULT_QOS_USE_SESSION_TIMERS)){
			mSession.setSessionTimer((long) mConfigurationService.getInt(
					SgsConfigurationEntry.QOS_SIP_CALLS_TIMEOUT,
					SgsConfigurationEntry.DEFAULT_QOS_SIP_CALLS_TIMEOUT),
					mConfigurationService.getString(SgsConfigurationEntry.QOS_REFRESHER,
							SgsConfigurationEntry.DEFAULT_QOS_REFRESHER));
        }
        // Precondition
		mSession.setQoS(tmedia_qos_stype_t.valueOf(mConfigurationService
				.getString(SgsConfigurationEntry.QOS_PRECOND_TYPE,
						SgsConfigurationEntry.DEFAULT_QOS_PRECOND_TYPE)),
				tmedia_qos_strength_t.valueOf(mConfigurationService.getString(SgsConfigurationEntry.QOS_PRECOND_STRENGTH,
						SgsConfigurationEntry.DEFAULT_QOS_PRECOND_STRENGTH)));
	    ...
	    mHistoryEvent = new SgsHistoryAVCallEvent((mediaType == SgsMediaType.AudioVideo || mediaType == SgsMediaType.Video), null);
	...
	private boolean initializeConsumersAndProducers(){
		...
		if((mediaMgr = super.getMediaSessionMgr()) != null){
			ProxyPlugin plugin;
			SgsProxyPlugin myProxyPlugin;
			// Video
			if(super.mMediaType == SgsMediaType.Video || super.mMediaType == SgsMediaType.AudioVideo){
				if((plugin = mediaMgr.findProxyPluginConsumer(twrap_media_type_t.twrap_media_video)) != null){
					if((myProxyPlugin = SgsProxyPluginMgr.findPlugin(plugin.getId())) != null){
						mVideoConsumer = (SgsProxyVideoConsumer)myProxyPlugin;
				...
				if((plugin = mediaMgr.findProxyPluginProducer(twrap_media_type_t.twrap_media_video)) != null){
					if((myProxyPlugin = SgsProxyPluginMgr.findPlugin(plugin.getId())) != null){
						mVideoProducer = (SgsProxyVideoProducer)myProxyPlugin;
			...
			// Audio
			if(super.mMediaType == SgsMediaType.Audio || super.mMediaType == SgsMediaType.AudioVideo){
				if((plugin = mediaMgr.findProxyPluginConsumer(twrap_media_type_t.twrap_media_audio)) != null){
					if((myProxyPlugin = SgsProxyPluginMgr.findPlugin(plugin.getId())) != null){
						mAudioConsumer = (SgsProxyAudioConsumer)myProxyPlugin;
				...
				if((plugin = mediaMgr.findProxyPluginProducer(twrap_media_type_t.twrap_media_audio)) != null){
					if((myProxyPlugin = SgsProxyPluginMgr.findPlugin(plugin.getId())) != null){
						mAudioProducer = (SgsProxyAudioProducer)myProxyPlugin;
	...
	// Doubango AEC for THIS session. Default value (for all sessions) is the one in "SgsConfigurationEntry.GENERAL_AEC" configuration entry.
	...
	public void setModeInCall(boolean bInCall){
		final AudioManager audiomanager = SgsApplication.getAudioManager();
		if(SgsApplication.isSetModeAllowed()){
