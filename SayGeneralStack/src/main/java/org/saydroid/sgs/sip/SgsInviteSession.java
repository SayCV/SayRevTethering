...
import org.saydroid.sgs.model.SgsDeviceInfo;
 ...
 * you own session.
 */
public abstract class SgsInviteSession extends SgsSipSession{
    ...
    private final SgsDeviceInfo mRemoteDeviceInfo;
     ...
     */
	public SgsInviteSession(SgsSipStack sipStack) {
		super(sipStack);
		
		mRemoteDeviceInfo = new SgsDeviceInfo();
		mState = InviteState.NONE;
	}
	
	protected abstract SgsHistoryEvent getHistoryEvent();
	 ...
	 */
     ...
     public void setState(InviteState state){
		mState = state;
		SgsHistoryEvent historyEvent = getHistoryEvent();
		switch (state) {
		case INCOMING:
			mEventIncoming = true;
			break;
		...
		case TERMINATING:
			if(historyEvent != null && !mEventAdded){
				...
				SgsEngine.getInstance().getHistoryService().addEvent(historyEvent);
			}
			break;
		}
     }
	...
	public SgsDeviceInfo getRemoteDeviceInfo(){
