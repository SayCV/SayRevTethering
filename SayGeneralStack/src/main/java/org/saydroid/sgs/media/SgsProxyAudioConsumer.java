 ...
 */
public class SgsProxyAudioConsumer extends SgsProxyPlugin{
	private static final String TAG = SgsProxyAudioConsumer.class.getCanonicalName();
	...
	public SgsProxyAudioConsumer(BigInteger id, ProxyAudioConsumer consumer){
		super(id, consumer);
	...
	public void setSpeakerphoneOn(boolean speakerOn){
		...
		if (SgsApplication.getSDKVersion() < 5){
		...
		else if(SgsApplication.useSetModeToHackSpeaker()){
		...
		if (super.mPrepared){
			mRoutingChanged = SgsApplication.isAudioRecreateRequired();
	...
	private boolean changeVolume(boolean bDown, boolean bVolumeChanged){
		...
		if(audioManager != null){
			if( !mIsInit && mAec && SgsApplication.getAudioManager().isSpeakerphoneOn() ){
				mIsInit = true ;
			...
			{
//				final float attenuation = mConfigurationService.getFloat(SgsConfigurationEntry.MEDIA_AUDIO_CONSUMER_ATTENUATION, 
//						SgsConfigurationEntry.DEFAULT_MEDIA_AUDIO_CONSUMER_ATTENUATION);
				final float attenuation = mConfigurationService.getFloat(SgsConfigurationEntry.GENERAL_AUDIO_PLAY_LEVEL ,
						SgsConfigurationEntry.DEFAULT_GENERAL_AUDIO_PLAY_LEVEL);
	...
	private synchronized int prepare(int ptime, int rate, int channels){
		...
		if(super.mValid && mConsumer != null && mInputRate != mOutputRate){
			if(mConsumer.queryForResampler(mInputRate, mOutputRate, mPtime, mChannels, 
					mConfigurationService.getInt(SgsConfigurationEntry.MEDIA_AUDIO_RESAMPLER_QUALITY, SgsConfigurationEntry.DEFAULT_MEDIA_AUDIO_RESAMPLER_QUALITY))){
		...
		mAec = mConfigurationService.getBoolean(SgsConfigurationEntry.GENERAL_AEC, SgsConfigurationEntry.DEFAULT_GENERAL_AEC) ;
	...
	private Runnable mRunnablePlayer = new Runnable(){
		...
		public void run() {
			...
			if(SgsProxyAudioConsumer.super.mValid){
				mConsumer.setPullBuffer(mOutputBuffer, mOutputBuffer.capacity());
				mConsumer.setGain(SgsEngine.getInstance().getConfigurationService().getInt(SgsConfigurationEntry.MEDIA_AUDIO_CONSUMER_GAIN, 
						SgsConfigurationEntry.DEFAULT_MEDIA_AUDIO_CONSUMER_GAIN));
			}
			
			while(SgsProxyAudioConsumer.super.mValid && SgsProxyAudioConsumer.super.mStarted){
				if(mAudioTrack == null){
					break;
				}
				...
				if(mRoutingChanged){
					...
					if(!SgsProxyAudioConsumer.super.mPaused){
						bPlaying = false;
						nWritten = 0;
					}
				}
	...
	{
		final SgsProxyAudioConsumer myConsumer;
		
		MyProxyAudioConsumerCallback(SgsProxyAudioConsumer consumer){
			super();
			myConsumer = consumer;
		}
