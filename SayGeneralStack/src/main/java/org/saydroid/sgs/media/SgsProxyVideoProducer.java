 ...
 */
public class SgsProxyVideoProducer extends SgsProxyPlugin{
	private static final String TAG = SgsProxyVideoProducer.class.getCanonicalName();
	...
	private static final boolean sAddCallbackBufferSupported = SgsCameraProducer.isAddCallbackBufferSupported();
	...
	public SgsProxyVideoProducer(BigInteger id, ProxyVideoProducer producer){
		super(id, producer);
        ...
        mWidth = SgsProxyVideoProducer.DEFAULT_VIDEO_WIDTH;
		mHeight = SgsProxyVideoProducer.DEFAULT_VIDEO_HEIGHT;
		mFps = SgsProxyVideoProducer.DEFAULT_VIDEO_FPS;
    }
	...
	public void toggleCamera(){
		if(super.mValid && super.mStarted && !super.mPaused && mProducer != null){
			final Camera camera = SgsCameraProducer.toggleCamera();
			try{
	...
	{
		if(super.mValid && super.mStarted && !super.mPaused && mProducer != null){
			final Camera camera = SgsCameraProducer.changeFrameRate(fps);
			try{
				startCameraPreview(camera);
			}
			catch (Exception exception) {
	...
	public int getTerminalRotation(){
		final android.content.res.Configuration conf = SgsApplication.getContext().getResources().getConfiguration();
	...
	public int getNativeCameraHardRotation(boolean preview){
		...
		if(SgsApplication.getSDKVersion() >= 9){			
			try {
				...
				int orientation = 0;
				int cameraId = 0;
				int numOfCameras = SgsCameraProducer.getNumberOfCameras();
				if (numOfCameras > 1) {
					if (SgsCameraProducer.isFrontFacingCameraEnabled()) {
				...
				Display display = SgsApplication.getDefaultDisplay();
				if (display != null) {
			...
			boolean isFront            = SgsCameraProducer.isFrontFacingCameraEnabled();
			if (SgsApplication.isSamsung() && !SgsApplication.isSamsungGalaxyMini()){
				if (preview){
					if (isFront){
			...
			else if (SgsApplication.isToshiba()){
				if (preview){
					if (terminalRotation == 0) return 0;
					else return 270;
				}
	...
	public boolean isFrontFacingCameraEnabled() {
		return SgsCameraProducer.isFrontFacingCameraEnabled();
	}
	...
	public void setOnPause(boolean pause){
		...
		try {
			if(super.mStarted){
				final Camera camera = SgsCameraProducer.getCamera();
				if(pause){
					camera.stopPreview();
				}
				else{
	...
	private synchronized int prepareCallback(int width, int height, int fps){
		Log.d(SgsProxyVideoProducer.TAG, "prepareCallback("+width+","+height+","+fps+")");
    ...
    private synchronized void startCameraPreview(Camera camera){
		if(camera != null && mProducer != null){
			...
			SgsCameraProducer.setDisplayOrientation(camera, rotation);
			
			// Callback Buffers
			if(SgsProxyVideoProducer.sAddCallbackBufferSupported){
				for(int i=0; i<SgsProxyVideoProducer.CALLABACK_BUFFERS_COUNT; i++){
					SgsCameraProducer.addCallbackBuffer(camera, new byte[mVideoFrame.capacity()]);
				}
			}
	...
	private PreviewCallback previewCallback = new PreviewCallback() {
	  public void onPreviewFrame(byte[] _data, Camera _camera) {
		  if(mStarted){
			  if(SgsProxyVideoProducer.super.mValid && mVideoFrame != null && _data != null){
				  mVideoFrame.put(_data);
				  mProducer.push(mVideoFrame, mVideoFrame.capacity());
				  mVideoFrame.rewind();
				}
			  if(SgsProxyVideoProducer.sAddCallbackBufferSupported){
				  SgsCameraProducer.addCallbackBuffer(_camera, _data);
			  }
		 }
	 }
	};
	...
	class MyProxyVideoProducerPreview extends SurfaceView implements SurfaceHolder.Callback {
		...
		private final SgsProxyVideoProducer myProducer;
		private Camera mCamera;
	
		MyProxyVideoProducerPreview(SgsProxyVideoProducer _producer) {
		...
		public void surfaceCreated(SurfaceHolder holder) {
			try {
				mCamera = SgsCameraProducer.openCamera(myProducer.mFps, 
						myProducer.mWidth, 
						myProducer.mHeight, 
						mHolder,
		...
		public void surfaceDestroyed(SurfaceHolder holder) {
			...
			try{
				SgsCameraProducer.releaseCamera(mCamera);
			}
			catch (Exception exception) {
				Log.e(TAG, exception.toString());
			}
    ...
    {
        final SgsProxyVideoProducer myProducer;
        public MyProxyVideoProducerCallback(SgsProxyVideoProducer producer){
        	super();
            myProducer = producer;
        }
