...
implements ITetheringService {
	...
	public boolean register(Context context) {
		...
		if (mTetheringStack == null) {
			...
			if (!mTetheringStack.setRealm(mPreferences.getRealm())) {
				Log.e(TAG, "Failed to set realm");
				return false;
			}
			if (!mTetheringStack.setIMPI(mPreferences.getIMPI())) {
				Log.e(TAG, "Failed to set IMPI");
				return false;
			}
			if (!mTetheringStack.setIMPU(mPreferences.getIMPU())) {
...
//			}
	...
	static class MySipCallback extends SipCallback{
		...
		public int OnDialogEvent(DialogEvent e){
			...
			TetheringSession mySession = null;
		...
		public int OnMessagingEvent(MessagingEvent e) {
			...
			switch(type){
				...
				case tsip_i_message:
					...
					if(_session == null){
						Log.e(TetheringService.TAG, "Failed to take session ownership");
						return -1;
					}
					...
					if(bytes == null || bytes.length ==0){
						Log.e(TetheringService.TAG, "Invalid MESSAGE");
						imSession.reject();
						imSession.decRef();
						return 0;
					}
                        ...
                        if (smsData != null){
                            ...
                            if (smsType == twrap_sms_type_t.twrap_sms_type_rpdata){
                                ...
                                if(origPhoneNumber != null){
                                	...
                                	Log.e(TetheringService.TAG, "Invalid destination address");
                                	return 0;
                                }
                                ...
                                if((SMSCPhoneNumber = SgsUriUtils.getValidPhoneNumber(SMSC)) == null){
                                	...
                                	if((SMSCPhoneNumber = SgsUriUtils.getValidPhoneNumber(SMSC)) == null){
                                		Log.e(TetheringService.TAG, "Invalid IP-SM-GW address");
                                		return 0;
                                	}
                                }
                                ...
                                if (payLength > 0) {
                                    ...
                                    if (rpError != null){
                                        ...
                                        if (err_len > 0){
                                            ...
                                            MessagingSession m = new MessagingSession(mTetheringService.getTetheringStack());
                                            m.setToUri(SMSC);
                                            m.addHeader("Content-Type", SgsContentType.SMS_3GPP);
                            	...
                            	if(smsType == twrap_sms_type_t.twrap_sms_type_ack){
                            		...
                            		Log.d(TetheringService.TAG, "RP-ACK");
                            	}
                            	else if(smsType == twrap_sms_type_t.twrap_sms_type_error){
                            		/* Find message from the history (by MR) an update it's status */
                            		Log.d(TetheringService.TAG, "RP-ERROR");
                            	}
                            }
                        }
					}
		...
		public int OnStackEvent(StackEvent e) {
			...
			switch(code){
				case tinyWRAPConstants.tsip_event_code_stack_started:
					mTetheringService.mSipStack.setState(STACK_STATE.STARTED);
					Log.d(TetheringService.TAG, "Stack started");
					break;
